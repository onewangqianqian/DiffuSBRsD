...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--validation_prompt",
        type=str,
        default=["A photo of <type-4-solar-burst>."],
        nargs="+",
        help=(
    ...
    parser.add_argument(
        "--validation_image",
        type=str,
        default=["examples/brushnet/test-image/image.jpg"],
        nargs="+",
        help=(
    ...
    parser.add_argument(
        "--validation_mask",
        type=str,
        default=["examples/brushnet/test-image/image1mask.png"],
        nargs="+",
        help=(
            "A set of paths to the paintingnet conditioning image be evaluated every `--validation_steps`"
            " and logged to `--report_to`. Provide either a matching number of `--validation_prompt`s, a"
...
class MyWebDataset():
    def __init__(self,resolution,tokenizer,random_mask,placeholder_token="*",learnable_property="object",):
    ...
    def coco2rle(self,mask_rle,shape):
        list=[]
        for index in mask_rle:
            for indey in index:
                list.append(indey)

        rle = maskUtils.frPyObjects(list, shape[0],  shape[1])
        if type(rle) == list:
    ...
    def polygon_to_mask(self,polygons,image_shape,  fill=0):
        ...
        image = Image.new('L', (image_shape[0],image_shape[1]), 1)  # 创建一个全黑的灰度图像
    ...
    def rle2mask(self, mask_rle, shape):# height width
        ...
        # mask_rle=self.polygon_to_mask(mask_rle,shape)
        # mask_rle=self.mask2rle(mask_rle)
    ...
    def rle2mask_burst(self,mask_rle,shape):
        img = np.ones(shape[0] * shape[1], dtype=np.uint8)
        points = np.array(mask_rle)
    ...
    def __call__(self,examples):
        ...
        for example in examples:
            ...
            image=cv2.imread('data/BurstData/cropped/'+example['img_name'])
...
def main(args):
    ...
    #                 split="train",
    #                 streaming=True)
    train_dataset = load_dataset("json", data_files="data/BurstData/solar_cropped_cls4.json",split='train',field='data')
    ...
    train_dataset_len=100000
    # train_dataset_len = 10000 * len(os.listdir(args.train_data_dir))
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=False,
        collate_fn=MyWebDataset(resolution = args.resolution,
                                tokenizer=tokenizer,
