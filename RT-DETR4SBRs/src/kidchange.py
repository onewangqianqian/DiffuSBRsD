...
class KernelInceptionDistance(Metric):
    ...
    def update(self, imgs: Tensor, real: bool) -> None:
        ...
        if real:
            ...
            self.fake_features.append(features[2])
    ...
    def compute(self) -> Tuple[Tensor, Tensor]:
        ...
        #     o = torch.norm(f_real - f_fake, p=2)
        #     kid_scores_.append(o)
        # diff_squared = (fake_features - real_features) ** 2
        # distances = torch.sqrt(diff_squared.sum(dim=1))
        # kid_scores = torch.stack(distances)
        # real_features_f=real_features.view(real_features.shape[0],-1)
        # fake_features_f = fake_features.view(fake_features.shape[0], -1)
        # cosine_similarity = 1 - F.cosine_similarity(fake_features_f, real_features_f, dim=1).abs()
        # mean=cosine_similarity.mean()
        # diff_squared = (fake_features - real_features) ** 2
        # distances = torch.sqrt(diff_squared.sum(dim=1))
        # mean=distances.mean()
        scores=[]
        for i in range(real_features.size(0)):
            # 计算每对图像的SSIM
            ssim_score = ms_ssim(fake_features, real_features[i].unsqueeze(0) if True else real_features[i], data_range=1.0,win_size=1).item()
            # 将SSIM分数添加到列表中
