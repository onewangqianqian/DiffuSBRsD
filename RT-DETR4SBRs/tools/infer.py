        ...
        for j, b in enumerate(box):
            if scrs[j] != 1:
                ...
                draw.text((b[0], b[1]), text=f"{dit_t[lab[j].item()]} {round(scrs[j].item(), 5)}",
                      font=ImageFont.load_default(45), fill='white',)
            else:
...
def main(args, ):
    ...
    # Caven = Image.new('RGBA', (620, 410), (0, 0, 0, 0))
    #创建一个透明背景的画布620*410
    ...
    for index in images:
        ...
        if dataset:
            ...
            path = args.distance_folder + os.path.basename(index).split('.')[0] + 'source.jpg'

            draw([im_pil], labels, boxes_tensor, scores, 0.5, path)
            print(path)
        else:
            path=args.distance_folder+os.path.basename(index).split('.')[0]+'.jpg'
            draw([im_pil], labels, boxes, scores, 0.7,path)
            # draw([im_pil], labels, boxes_tensor, scores, 0.5, path)
