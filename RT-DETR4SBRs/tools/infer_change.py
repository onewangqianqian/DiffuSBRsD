...
def main(args, ):
    ...
    real_images=get_json_image(args.im_file+"\duibi")
    # real_tensors=[]
    # mid_features=[]
    ...
    distance_folder=args.im_file
    textnumber= len(get_text(args.im_file))
    for number,index in enumerate(imagelist[textnumber:]):
        ...
        midoutput = model(im_data, None, mid_features=True)[2]
        score_list=[]
        for data in real_images:
            temp_pil=Image.open(data).convert('RGB')
            temp_data=transforms(temp_pil)[None].to(args.device)
            real_outputs=model(temp_data, None, mid_features=True)[2]
            ssim_score = ms_ssim(midoutput, real_outputs,data_range=1.0, win_size=1).item()
            score_list.append(ssim_score)
        kid_mean=max(score_list)
