...
@TRANSFORMER.register_module()
class DeformableDetrTransformer_conv(Transformer):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(DeformableDetrTransformer_conv, self).__init__(**kwargs)
    ...
    def init_layers(self):
        ...
        self.DynamicConv_D0=DynamicConv_D(in_planes=256,out_planes=256,kernel_size=3,stride=1,padding=1,bias=False)
        self.DynamicConv_D1 = DynamicConv_D(in_planes=256, out_planes=256, kernel_size=3, stride=1, padding=1,
                                            bias=False)
        self.DynamicConv_D2 = DynamicConv_D(in_planes=256, out_planes=256, kernel_size=3, stride=1, padding=1,
                                            bias=False)
        self.DynamicConv_D3 = DynamicConv_D(in_planes=256, out_planes=256, kernel_size=3, stride=1, padding=1,
                                            bias=False)
    def unclose_conv_close(self,outs):

        num_conv = len(outs)
        for lvl in range(num_conv):
            outs[lvl] =getattr(self,f"DynamicConv_D{lvl}")(outs[lvl])
    ...
    def forward(self,
        ...
        memory = self.unclose_conv_close(outs)# 修改的 位置
