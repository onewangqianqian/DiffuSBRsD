...
class DetCAMVisualizer:
    ...
    def show_cam(self,
        ...
        if with_norm_in_bboxes is True:
            ...
            img[:,:]=scale_cam_image(grayscale_cam[:,:].copy())
            images.append(img)
    ...
    def _draw_boxes(self, boxes, labels, image):
        for i, box in enumerate(boxes):
            ...
            #               (int(box[2]), int(box[3])), color, 5)
            # x0=int((int(box[0])+int(box[2]))/2)
            # x1=int((int(box[1])+int(box[3]))/2)
            # radius = 3
            # # cv2.circle(image, (x0,x1), radius, color, -1)
            # color = [0, 0, 0]
            # cv2.putText(
            #     image,
