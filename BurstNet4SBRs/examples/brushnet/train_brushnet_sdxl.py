...
# ]
imagenet_templates_small = [
    "a photo of a {} on a blue background",
    "a rendering of a {} on a blue background",
    "the photo of a {} on a blue background",
]
...
# ]
def log_validation( text_encoder_1,
    text_encoder_2,
    tokenizer_1,
    tokenizer_2,vae, unet, brushnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image, validation_mask in zip(validation_prompts, validation_images, validation_masks):
        ...
        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                    prompt=validation_prompt, image=validation_image, mask=validation_mask, num_inference_steps=20, generator=generator
                ).images[0]
            images.append(image)

        image_logs.append(
            {"validation_image": validation_image, "images": images, "validation_prompt": validation_prompt,"seed":args.seed}
        )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--placeholder_token",
        type=str,
        default="<type4-one-solar-radio-burst>",
        required=True,
    ...
    parser.add_argument(
        "--image_path",
        type=str,
        default='data/BurstData/type4_G_1',
        help=(
            ""
        ),
    )
...
class MyWebDataset():
    def __init__(self,resolution,text_encoders, tokenizers,  random_mask,image_path,placeholder_token="*",learnable_property="object",):
    ...
    def polygon_to_mask(self,polygons,image_shape,  fill=0):
        """
        将多边形坐标转换为二值掩码。

        参数:
        - image_shape: 元组，表示图像的尺寸 (height, width)。
        - polygons: 列表，包含多个多边形的坐标，每个多边形是一个包含(x, y)坐标的列表。
        - fill: 用于填充多边形的像素值（默认为255，即白色）。

        返回:
        - mask: 二值NumPy数组，其中多边形内部为fill值，外部为0。
        """
        # polygons_temp=[[]]
        # for index in polygons:
        #     tuple=(index[0],index[1])
        arr = np.ones((image_shape[1], image_shape[0]), dtype=np.uint8)

        for index in polygons:

            polygon_points = np.array(index["points"], dtype=np.int32)
            polygon_points = polygon_points.reshape((-1, 1, 2))
            cv2.fillPoly(arr, [polygon_points], 0)
        # x_min = min(p[0] for p in polygons)
        # x_max = max(p[0] for p in polygons)
        # y_min = min(p[1] for p in polygons)
        # y_max = max(p[1] for p in polygons)
        # arr[y_min:y_max + 1,x_min:x_max + 1] = 0
        return arr.reshape((image_shape[1],image_shape[0],1),order='F')
    ...
    def tokenize_captions(self, caption, original_size, crops_coords_top_left, target_size, is_train=True):
        prompt_embeds, pooled_prompt_embeds = encode_prompt(
            [caption], self.text_encoders, self.tokenizers, args.proportion_empty_prompts, is_train
        )
    ...
    def __call__(self,examples):
        ...
        for example in examples:
            ...
            caption_path=self.image_path + example['img_name'].replace('.jpg', '.txt')
            with open(caption_path, 'r', encoding='utf-8') as file:
                caption=file.read()
            height = int(example["height"])
            width = int(example["width"])
            image = cv2.imread(self.image_path + example['img_name'])
            segmentation = example["segmentation"]

            # if len(segmentation["mask"])>0:
            #     mask=self.polygon_to_mask(segmentation,(width,height))
            if segmentation:
                mask=self.polygon_to_mask(segmentation,(width,height))
            else:
            ...
            # modified_mask_path = 'output/path_to_modified_mask_image_test.png'
            # cv2.imwrite(modified_mask_path, mask)
            # cv2.imwrite(modified_mask_path, image)
            masked_image=image*mask
            # cv2.imwrite(modified_mask_path, masked_image)
            # print(f"Modified mask saved to {modified_mask_path}")
            ...
            random_crop=[random.randint(0,h_new-self.resolution),random.randint(0,w_new-self.resolution)]
            crops_coords_top_left = (random_crop[0], random_crop[1])
...
def main(args):
    ...
    if accelerator.mixed_precision == "fp16":
        vae = AutoencoderKL.from_pretrained("data/ckpt/juggernautXL_juggernautX/vae", torch_dtype=torch.float16)
    else:
        vae = AutoencoderKL.from_pretrained(
            vae_path,
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                #
                #     i -= 1
                sub_dir1="text_encoder"
                models[0].save_pretrained(os.path.join(output_dir, sub_dir1))
                sub_dir2="brushnet"
                models[1].save_pretrained(os.path.join(output_dir, sub_dir2))
    ...
    # Optimizer creation
    params_to_optimize_1 = brushnet.parameters()
    params_to_optimize_2=text_encoder_one.get_input_embeddings().parameters()
    params_to_optimize=list(params_to_optimize_1)+list(params_to_optimize_2)
    ...
    train_dataset = load_dataset("json", data_files=args.image_path+"/data.json",split='train',field='data')
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=False,
        collate_fn=MyWebDataset(resolution = args.resolution, text_encoders = text_encoders, tokenizers = tokenizers, placeholder_token=placeholder_token,random_mask=args.random_mask,image_path=args.image_path),
    ...
    text_encoder_one,brushnet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        text_encoder_one,brushnet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                if args.pretrained_vae_model_name_or_path is None:
                    conditioning_latents = conditioning_latents.to(weight_dtype)
                ...
                #     text_encoder_one(batch["prompt_embeds"][:,:,:768], output_hidden_states=True)
                #     .hidden_states[-2]
                #     .to(dtype=weight_dtype)
                # )
                # encoder_output_2 = text_encoder_two(
                #     batch["prompt_embeds"][:, :, 768:].reshape(batch["prompt_embeds"][:,:,:768].shape[0], -1), output_hidden_states=True
                # )
                # encoder_hidden_states_2 = encoder_output_2.hidden_states[-2].to(dtype=weight_dtype)
                #
                # added_cond_kwargs = {"text_embeds": encoder_output_2[0], "time_ids": batch["add_time_ids"]}
    ...
    if accelerator.is_main_process:
        ...
        # Setting `vae`, `unet`, and `brushnet` to None to load automatically from `args.output_dir`.
        image_logs = None
