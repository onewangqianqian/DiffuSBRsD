...
def log_validation(vae, unet, brushnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image, validation_mask in zip(validation_prompts, validation_images,
                                                                    validation_masks):
        ...
        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                    prompt=validation_prompt, image=validation_image, mask=validation_mask, num_inference_steps=20,
                    generator=generator
                ).images[0]
            images.append(image)
...
class MyWebDataset():
    def __init__(self, resolution, text_encoders, tokenizers,image_path, random_mask):
...
def main(args):
    ...
    train_dataset = load_dataset("json", data_files="data/BurstData/type4-one.json",split='train',field='data')

    train_dataset_len = 50
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=False,
        collate_fn=MyWebDataset(resolution = args.resolution, text_encoders = text_encoders, tokenizers = tokenizers, random_mask=args.random_mask,image_path='data/BurstData/type4_G_1/'),
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                batch["unet_added_conditions"] = {
                    "text_embeds": batch["unet_added_conditions"]["text_embeds"].to(weight_dtype),
