    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            if len(segmentation["mask"]) > 0:
                temp=random.choice(segmentation["mask"])
                mask = self.rle2mask(temp, (height, width))[:, :, np.newaxis]
            else:
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                conditioning_latents = vae.encode(
                    batch["conditioning_pixel_values"].to(dtype=weight_dtype)).latent_dist.sample()
